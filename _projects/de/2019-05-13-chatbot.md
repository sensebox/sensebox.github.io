...
Als erstes wollen wir versuchen die rote Onboard-LED auf unserem Controller über den Bot zu steuern. Als Grundlage nutzen wir dazu das FlashledBot Beispiel aus der UniversalTelegramBot Library (Das Original findet ihr über "Datei > Beispiele > Beispiele aus eigenen Bibiliotheken > UniversalTelegramBot > 101 > FlaschledBot"). Im folgenden wurde zunächst lediglich den WLAN Zugang (ssid und password), den Access Token (BOTtoken) und die Nummer des LED-Pins (7 für die rote LED auf dem Board) angepasst:
...
Jetzt ladet ihr den Code auf das senseBox Board hoch und wechselt zu Telegram. Hier könnt ihr nun mit dem Bot chatten indem ihr seinen Kontakt auswählt (entweder ihr klickt auf den Link den ihr von BotFather erhalten habt, oder ihr sucht den Bot über das Suchfeld). Nun könnt ihr anfangen zu chatten! Mit dem Befehl `/start` bekommt ihr eine Übersicht über die verfügbaren Befehle:
...
Im seriellen Monitor solltet jedes Mal eine Benachrichtigung bekommen, wenn der Bot eine Nachricht bekommt. 

{% include image.html image=page.image6 %}

Schauen man im Sketch die Funktion `handleNewMessages(…)` ab Zeile 23 an sieht man, dass dort alle verfügbaren Kommandos die der Bot kennt definiert werden. Man kann hier die bekannten Kommandos ändern bzw. neu erstellen oder löschen. 
